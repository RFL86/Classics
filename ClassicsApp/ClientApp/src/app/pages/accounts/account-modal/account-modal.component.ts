...
import { Address } from './account-modal.model';
...
export class AccountModalComponent implements OnInit, AfterViewInit {
  ...
  constructor(public _activeModal: NgbActiveModal, public _service: AccountModalService) {
    ...
    this._service.editUser(this.formData).subscribe(
      result => {
        if (result == null || result == '') {
          ...
          this.successMessage = 'Usuário editado com sucesso.';
          this.loaderData = { visible: false, text: "" };
          setTimeout(() => {
            this.successMessage = '';
            this.showSuccessMessage = false;
